...
const AnimatedText = ({text, className=''}) => {
  return (
    ...
    overflow-hidden dark:text-light sm:py-0'>
        <motion.h1 className={`${className} inline-block w-full text-dark font-bold text-8xl dark:text-light`}
        variants={quote} initial='initial' animate='animate'>
        {
          text.split(" ").map((word,index)=>
            <motion.span key={word+'-'+index} className='inline-block dark:text-light'
            variants={singleWord}>
              {word}&nbsp;
            </motion.span>
          )
